...
class q1c_problem:
    ...
    def __init__(self, gameState: GameState):
        ...
        self.walls=[]
        self.goalmap=[]
        self.finalreward = 500
        self.possible_node = 0
    ...
    def BFS(self):
        ...
        queue = []
        start = []
        start.append(self.current[0])
        start.append(self.current[1])
        start.append(0)
        queue.append(start)
        flag = 0
        while 1:
            ...
            if self.goalmap[node[0]][node[1]]:
                ...
                if node[0]- 1 >= 0 and  self.walls[node[0]-1][node[1]] == False and visited[node[0]-1][node[1]]==0:
                    queue.append([node[0]-1,node[1],node[2]+1])
                    visited[node[0]-1][node[1]] = 1
                
                if node[1]-1 >=0 and  self.walls[node[0]][node[1]-1] == False and visited[node[0]][node[1]-1]==0:
                    queue.append([node[0],node[1]-1,node[2]+1])
                    visited[node[0]][node[1]-1] = 1 
                if node[1]+1 < self.goalmap.height and self.walls[node[0]][node[1]+1] == False and visited[node[0]][node[1]+1]==0:
                    queue.append([node[0],node[1]+1,node[2]+1])
                    visited[node[0]][node[1]+1] = 1  
                if node[0]+1 < self.goalmap.width and self.walls[node[0]+1][node[1]] == False and visited[node[0]+1][node[1]] == 0:
                    queue.append([node[0]+1,node[1],node[2]+1])
                    visited[node[0]+1][node[1]] = 1 
            queue.pop(0)
    ...
    def getSuccessors(self, state):
        ...
        y = state.current[1]
        #West
        if x-1 >=0 and state.walls[x-1][y] == 0:
            result["West"] = [x-1,y]
        #East
        if x+1 < state.goalmap.width and state.walls[x+1][y] == 0:
            result["East"] = [x+1,y]
        #North
        if y+1 < state.goalmap.height and state.walls[x][y+1] == 0:
            result["North"] = [x,y+1]
        #South
        if y-1 >= 0 and state.walls[x][y-1] == 0:
            result["South"] = [x,y-1]
        return result
        #util.raiseNotDefined()
